...
class user_manager_model extends MY_Model {
	...
	*/
	...
	public function login($email = "", $password = "", $remember = false){
		...
		validate("null", __("The account you entered does not match any account"), $user);

		$team = $this->model->get("*", $this->tb_team, "owner = '{$user->id}'");

		validate("null", __("The is a problem on your account. Please try again later"), $team);
		...
		try {
			switch ($type) {
				case 'facebook':
			        ...
			        $app_version = get_option('facebook_login_app_version', 'v4.0');
